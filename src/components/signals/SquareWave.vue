<template>
  <div class="square-wave-generator bg-gray-800/50 rounded-lg p-3 border border-gray-700">
    <div class="flex items-center justify-between mb-3">
      <span class="text-xs font-bold text-yellow-300">SQUARE WAVE</span>
      <button @click="toggleOscillator" :class="[
        'px-3 py-1 text-[0.6rem] font-bold rounded transition-colors',
        isPlaying ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-400'
      ]">
        {{ isPlaying ? 'ON' : 'OFF' }}
      </button>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3">
      <Knob v-model="frequency" :min="20" :max="20000" :step="1" label="Frequency" unit="Hz" color="#eab308" />
      <Knob v-model="volume" :min="-60" :max="0" :step="1" label="Volume" unit="dB" color="#eab308" />
    </div>
  </div>
</template>
