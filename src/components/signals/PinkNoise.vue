<template>
  <div class="pink-noise-generator bg-gray-800/50 rounded-lg p-3 border border-gray-700">
    <div class="flex items-center justify-between mb-3">
      <span class="text-xs font-bold text-pink-300">PINK NOISE</span>
      <button @click="toggleNoise" :class="[
        'px-3 py-1 text-[0.6rem] font-bold rounded transition-colors',
        isPlaying ? 'bg-pink-600 text-white' : 'bg-gray-700 text-gray-400'
      ]">
        {{ isPlaying ? 'ON' : 'OFF' }}
      </button>
    </div>
    ...
    <div class="flex flex-col gap-3">
      <Knob v-model="volume" :min="-60" :max="0" :step="1" label="Volume" unit="dB" color="#ec4899" />
    </div>
  </div>
</template>
