<template>
  <div class="compressor-effect bg-gray-800/50 rounded-lg p-3 border border-gray-700 flex flex-col items-center gap-2">
    <span class="text-xs font-bold text-green-300">COMPRESSOR</span>

    <button @click="toggleEffect" :class="[
      'px-4 py-1 text-sm font-bold rounded transition-colors w-full',
      isEnabled
    ...
    <button @click="showModal = true"
      class="px-4 py-1 text-sm font-bold rounded bg-blue-600 hover:bg-blue-700 text-white w-full">
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50"
        ...
        <div class="bg-gray-900 rounded-lg border-2 border-green-600 p-6 max-w-2xl w-full mx-4" @click.stop>
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold text-green-300">Compressor Settings</h3>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Knob v-model="threshold" :min="-60" :max="0" :step="1" label="Threshold" unit="dB" color="#10b981" />

            <Knob v-model="ratio" :min="1" :max="20" :step="0.1" label="Ratio" unit=":1" color="#10b981" />

            <Knob v-model="attack" :min="0.001" :max="0.1" :step="0.001" label="Attack" unit="s" color="#10b981" />

            <Knob v-model="release" :min="0.01" :max="1" :step="0.01" label="Release" unit="s" color="#10b981" />
          </div>
        </div>
      </div>
    </Teleport>
...
const threshold = ref(props.initialThreshold ?? -24)
const ratio = ref(props.initialRatio ?? 4)
const attack = ref(props.initialAttack ?? 0.003)
...
function startMonitoring() {
  ...
  function updateLevels() {
    ...
    // Get input level from masterSection meters
    if (props.masterSection?.getMeterValues) {
      try {
        const meters = props.masterSection.getMeterValues()
...
function drawCompressionCurve() {
  ...
  // Helper to map dB to canvas coordinates
  ...
  const knee = 6 // Soft knee width in dB
  ...
  if (currentInputLevel.value > -60) {
    ...
    if (isEnabled.value) {
      // Only apply compression if effect is enabled
      if (inputDb > thresholdValue - knee / 2) {
        if (inputDb > thresholdValue + knee / 2) {
          const excess = inputDb - thresholdValue
          outputDb = thresholdValue + excess / ratioValue
