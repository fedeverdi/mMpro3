                  ...
                  <div v-for="scene in scenes" :key="scene.name"
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        ...
                        <span v-if="currentSceneName === scene.name"
                    ...
                    <div class="flex items-center gap-2 ml-4">
                      <button @click="handleLoadScene(scene.name)"
                        class="px-3 py-1 text-xs bg-green-600 hover:bg-green-500 text-white rounded font-semibold transition-colors">
                        Load
                      </button>
                      <button v-if="currentSceneName === scene.name" @click="updateCurrentScene(scene)"
                        class="px-3 py-1 text-xs bg-blue-600 hover:bg-blue-500 text-white rounded font-semibold transition-colors">
                        Update
                      </button>
                      <button @click="handleRenameScene(scene)"
                        class="px-3 py-1 text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 rounded font-semibold transition-colors">
                        Rename
                      </button>
                      <button @click="handlePinScene(scene)"
                        :class="['px-3 py-1 text-xs rounded font-semibold transition-colors', scene.pinned ? 'bg-yellow-500 hover:bg-yellow-400 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300']"
                        :title="scene.pinned ? 'Unpin scene' : 'Pin scene'">📌
                      </button>
...
const { scenes, currentSceneName, saveScene, loadScene, loadAllScenes, deleteScene, renameScene, pinScene } = useScenes()
...
async function handleLoadScene(sceneName: string) {
  try {
    ...
    notify.success(`Scene "${sceneName}" loaded`)
  } catch (error) {
    console.error('[ScenesModal] Error loading scene:', error)
    notify.error('Failed to load scene')
  }
}

async function updateCurrentScene(scene: any) {
  // Confirm update with custom notification
  const confirmed = await notify.confirm(`Update "${scene.name}" with current state?`)
  if (!confirmed) return

  try {
    // Re-save with same name to update (Rust overwrites)
    await saveScene(scene.name)
    await loadAllScenes()
    notify.success(`Scene "${scene.name}" updated`)
    close()
  } catch (error) {
    console.error('[ScenesModal] Error updating scene:', error)
    notify.error('Failed to update scene')
  }
}
...
async function confirmDeleteScene(scene: any) {
  ...
  try {
    await deleteScene(scene.name)
    notify.success(`Scene "${scene.name}" deleted`)
  } catch (error) {
    console.error('[ScenesModal] Error deleting scene:', error)
    notify.error('Failed to delete scene')
  }
}

async function handleRenameScene(scene: any) {
  const newName = window.prompt(`Rename scene "${scene.name}":`, scene.name)
  if (!newName || newName === scene.name) return

  try {
    await renameScene(scene.name, newName)
    await loadAllScenes()
    notify.success(`Scene renamed to "${newName}"`)
  } catch (error) {
    console.error('[ScenesModal] Error renaming scene:', error)
    notify.error('Failed to rename scene')
  }
}

async function handlePinScene(scene: any) {
  try {
    await pinScene(scene.name)
  } catch (error) {
    console.error('[ScenesModal] Error pinning scene:', error)
    notify.error('Failed to pin scene')
  }
}
