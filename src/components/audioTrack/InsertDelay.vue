<template>
  ...
  <div @click="handleToggle" :class="[
    ...
    enabled ? 'bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'
  ]">
    ...
    <div class="flex items-center gap-0.5">
      <button :disabled="!enabled" @click.stop="showModal = true"
        class="p-0.5 rounded hover:bg-blue-700 disabled:opacity-50">
  ...
  <Teleport to="body">
    <div v-if="showModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-[1000]"
      ...
      <div class="bg-gray-900 rounded-lg border-2 border-blue-600 p-6 max-w-2xl w-full mx-4" @click.stop>
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-bold text-blue-300">Track {{ trackNumber }} - Insert #{{ insertId }}</h3>
          <button @click="showModal = false" class="text-gray-400 hover:text-white text-2xl">&times;</button>
        </div>
        <div class="flex flex-wrap gap-4 justify-center">
          <Knob v-model="timeL" :min="0" :max="2000" :step="1" label="Time L" unit="ms" color="#3b82f6" />
          <Knob v-model="timeR" :min="0" :max="2000" :step="1" label="Time R" unit="ms" color="#06b6d4" />
          <Knob v-model="feedback" :min="0" :max="95" :step="1" label="Feedback" unit="%" color="#f59e0b" />
          <Knob v-model="mix" :min="0" :max="100" :step="1" label="Mix" unit="%" color="#10b981" />
        </div>
      </div>
    </div>
  </Teleport>
...
// Delay parameters
const timeL = ref(250)  // milliseconds (0-2000)
const timeR = ref(250)  // milliseconds (0-2000)
const feedback = ref(30)  // 0-95% (converted to 0.0-0.95 for backend)
const mix = ref(30)  // 0-100% (converted to 0.0-1.0 for backend)
...
watch([timeL, timeR, feedback, mix], () => {
  if (audioEngine?.state.value.isRunning && props.enabled) {
    audioEngine.setTrackInsertDelay(
      props.trackNumber - 1,
      props.insertId,
      timeL.value,
      timeR.value,
