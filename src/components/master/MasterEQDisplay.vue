<template>
  <div class="master-eq-track relative bg-gradient-to-b from-gray-900 to-gray-950 border border-blue-500/70 p-2 flex flex-col gap-3 h-full">
    ...
    <div class="relative w-full flex-1 min-h-0 rounded border border-gray-800 bg-black/50 overflow-hidden shadow-inner">
      <canvas
        ref="masterEqCanvas"
...
async function loadEQPresetsFromBackend() {
  try {
    ...
    console.error('[MasterEQDisplay] Failed to load EQ presets from backend:', error)
  }
}
...
function drawMasterEQPreview() {
  ...
  const freqs = [20, 30, 40, 50, 60, 80, 100, 200, 300, 400, 500, 600, 800, 1000, 2000, 5000, 10000, 20000]
  freqs.forEach(freq => {
